...
const getRequestObj = (pageIndex, pageSize, filter) => ({
  all: () => ({
    pageIndex,
    pageSize,
    filter
...
test("make sure users are returned properly", async ({ assert }) => {
  const userData = await uc.getUsers({ request: getRequestObj(0, 3) });
...
test("make sure pagination works", async ({ assert }) => {
  let userData = await uc.getUsers({ request: getRequestObj(1, 3) });
  assert.deepEqual(userData, {
    total: 30,
    perPage: 3,
    page: 2,
  ...
  userData = await uc.getUsers({ request: getRequestObj(2, 2) });
  assert.deepEqual(userData, {
    total: 30,
    perPage: 2,
    page: 3,
  ...
  userData = await uc.getUsers({ request: getRequestObj(10, 5) });
  assert.deepEqual(userData, {
    total: 30,
    perPage: 5,
    page: 11,
...
test("make sure filtering works", async ({ assert }) => {
  ...
  let userData = await uc.getUsers({ request: getRequestObj(0, 10, "luis") });
  assert.deepEqual(userData, {
    total: 1,
    perPage: 10,
    page: 0,
  ...
  userData = await uc.getUsers({ request: getRequestObj(0, 10, "fa") });
  assert.deepEqual(userData, {
    total: 2,
    perPage: 10,
    page: 0,
  ...
  userData = await uc.getUsers({ request: getRequestObj(0, 10, "17/07") });
  ...
  userData = await uc.getUsers({ request: getRequestObj(0, 10, "bo@") });
  console.log(userData);
  assert.deepEqual(userData, {
    total: 1,
    perPage: 10,
